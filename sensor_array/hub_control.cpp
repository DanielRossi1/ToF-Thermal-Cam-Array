...
static bool parseMode(const char* v, StreamMode& out) {
  ...
  if (streqi(v, "tof") || streqi(v, "tofonly")) {
    out = StreamMode::TofOnly;
    return true;
  }
  if (streqi(v, "mlx") || streqi(v, "mlxonly")) {
    out = StreamMode::MlxOnly;
    return true;
  }
  if (streqi(v, "cam") || streqi(v, "camonly")) {
    out = StreamMode::CamOnly;
    return true;
  }
  if (streqi(v, "none") || streqi(v, "off")) {
    out = StreamMode::None;
    return true;
  }
  return false;
...
static void cmdHelp(Transport& tx, uint32_t seq) {
  tx.sendTextResp(seq,
                  ...
                  "  SET CAMSYNC ... (only if HUB_USE_CAM_SYNC=1)\n"
                  "  DIAG I2C SCAN\n"
...
static uint8_t parseMlxMode(const char* v, uint8_t fallback) {
...
static uint8_t parseMlxRes(const char* v, uint8_t fallback) {
  if (!v) return fallback;
...
static uint8_t parseMlxRefresh(const char* v, uint8_t fallback) {
  if (!v) return fallback;
...
static void handleCmdText(ControlContext& ctx, Transport& tx, uint32_t seq, char* cmd) {
  trim(cmd);
  if (!*cmd) {
    tx.sendTextResp(seq, "ERR empty\n");
    return;
  }
  ...
  if (streqi(t0, "get")) {
    ...
    if (what && streqi(what, "info")) {
      ...
      snprintf(buf, sizeof(buf),
               ...
               modeToStr(g_mode),
               (unsigned)ts.side, (unsigned)ts.ranging_hz, (unsigned)ts.integration_time_ms, (unsigned)ts.continuous,
               (unsigned)ms.mode, (unsigned)ms.resolution, (unsigned)ms.refresh,
  ...
  if (streqi(t0, "stream")) {
    ...
    for (;;) {
      ...
      if (streqi(kv, "enable")) {
        uint32_t en = 0;
        if (!parseU32(v, en)) {
          tx.sendTextResp(seq, "ERR invalid enable\n");
          return;
        }
        ...
        StreamMode m;
        if (!parseMode(v, m)) {
          tx.sendTextResp(seq, "ERR invalid mode\n");
          return;
        }
        g_mode = m;
      }
    ...
    if (!saw_any) {
      tx.sendTextResp(seq, "ERR stream expects enable=... and/or mode=...\n");
      return;
    }
  ...
  if (streqi(t0, "set")) {
    ...
    if (!which) {
      tx.sendTextResp(seq, "ERR set expects TOF|MLX|CAM|CAMSYNC\n");
      return;
    }
    ...
    if (streqi(which, "i2c")) {
      ...
      for (;;) {
        ...
        const char* v = kvValue(kv);
        if (!v) continue;
        if (streqi(kv, "clock_hz")) {
          (void)parseU32(v, clock_hz);
        }
      }

      if (!clock_hz) {
        tx.sendTextResp(seq, "ERR i2c expects clock_hz=...\n");
        return;
      }

      I2CLock lk(*ctx.i2c, pdMS_TO_TICKS(50));
      if (!lk.ok()) {
        tx.sendTextResp(seq, "ERR i2c busy\n");
        return;
      }
      ctx.i2c->wire().setClock(clock_hz);
      tx.sendTextResp(seq, "OK\n");
      return;
    }
    ...
    if (streqi(which, "tof")) {
      ...
      for (;;) {
        ...
        uint32_t n = 0;
        if (streqi(kv, "side")) {
          if (parseU32(v, n)) s.side = (uint8_t)n;
        } else if (streqi(kv, "hz")) {
          if (parseU32(v, n)) s.ranging_hz = (uint16_t)n;
        } else if (streqi(kv, "it_ms")) {
          if (parseU32(v, n)) s.integration_time_ms = (uint16_t)n;
    ...
    if (streqi(which, "mlx")) {
      ...
      for (;;) {
        ...
        if (streqi(kv, "mode")) {
          s.mode = parseMlxMode(v, s.mode);
        } else if (streqi(kv, "res")) {
          s.resolution = parseMlxRes(v, s.resolution);
        } else if (streqi(kv, "refresh")) {
          s.refresh = parseMlxRefresh(v, s.refresh);
        }
      }
    ...
    if (streqi(which, "camsync")) {
      ...
      for (;;) {
        ...
        if (!v) continue;
        uint32_t n = 0;
        if (streqi(kv, "enabled")) {
          if (parseU32(v, n)) s.enabled = (n != 0);
        } else if (streqi(kv, "period_us")) {
          if (parseU32(v, n)) s.trigger_period_us = n;
        } else if (streqi(kv, "pulse_us")) {
          if (parseU32(v, n)) s.trigger_pulse_us = n;
        }
      }
    ...
    if (streqi(which, "cam")) {
      ...
      for (;;) {
        ...
        if (streqi(kv, "enable")) {
          ...
          if (parseU32(v, n)) s.w = n;
        } else if (streqi(kv, "h")) {
          if (parseU32(v, n)) s.h = n;
        } else if (streqi(kv, "interval_us")) {
          if (parseU32(v, n)) s.interval_us = n;
        }
      }
  ...
  if (streqi(t0, "diag")) {
    ...
    if (what && streqi(what, "i2c")) {
      ...
      if (sub && streqi(sub, "scan")) {
        ...
        char out[512];
        size_t n = 0;
        n += snprintf(out + n, sizeof(out) - n, "OK i2c_scan\\n");
        for (uint8_t addr = 1; addr < 127; addr++) {
          ctx.i2c->wire().beginTransmission(addr);
          uint8_t err = ctx.i2c->wire().endTransmission();
          if (err == 0) {
    ...
    tx.sendTextResp(seq, "ERR unknown DIAG (try: DIAG I2C SCAN)\n");
    return;
  }
...
void handleSlipFrame(const uint8_t* data, size_t len, void* user) {
  ...
  // Validate minimum sizes: header + crc
  ...
  uint32_t crc = 0;
  crc = esp_crc32_le(crc, (const uint8_t*)&hdr, sizeof(hdr));
  if (hdr.payload_len) {
  ...
  // Payload is a UTF-8 command line. Copy into a mutable buffer.
  static char cmd_buf[256];
  const size_t n = (hdr.payload_len < sizeof(cmd_buf) - 1) ? hdr.payload_len : (sizeof(cmd_buf) - 1);
