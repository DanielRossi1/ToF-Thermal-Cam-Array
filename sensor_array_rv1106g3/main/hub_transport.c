...
// termios2 / BOTHER may not be in userspace headers even when
// TCGETS2/TCSETS2 ioctl numbers are.  Provide a local definition.
...
static void slip_write_byte(Transport *t, uint8_t b) {
    uint8_t esc_seq[2];
    switch (b) {
        case SLIP_END:
            esc_seq[0] = SLIP_ESC; esc_seq[1] = SLIP_ESC_END;
            write(t->fd, esc_seq, 2);
            break;
        case SLIP_ESC:
            esc_seq[0] = SLIP_ESC; esc_seq[1] = SLIP_ESC_ESC;
            write(t->fd, esc_seq, 2);
            break;
        default:
            write(t->fd, &b, 1);
...
static void slip_write(Transport *t, const uint8_t *data, size_t len) {
    for (size_t i = 0; i < len; i++)
        slip_write_byte(t, data[i]);
}
...
void transport_send(Transport *t, int type, uint32_t seq, uint64_t ts_us,
    ...
    if (payload_len && payload)
        crc = simple_crc32(crc, (const uint8_t *)payload, payload_len);

    // Worst-case SLIP size: 2 + 2*(hdr+payload+crc)
    ...
    // encode helper inline
    #define SLIP_PUSH(byte) do { \
        uint8_t _b = (byte); \
        if      (_b == SLIP_END) { buf[i++] = SLIP_ESC; buf[i++] = SLIP_ESC_END; } \
        else if (_b == SLIP_ESC) { buf[i++] = SLIP_ESC; buf[i++] = SLIP_ESC_ESC; } \
        else                     { buf[i++] = _b; } \
    } while(0)

    const uint8_t *h = (const uint8_t *)&hdr;
    for (size_t k = 0; k < sizeof(hdr); k++) SLIP_PUSH(h[k]);
    if (payload_len && payload) {
        const uint8_t *p = (const uint8_t *)payload;
        for (uint32_t k = 0; k < payload_len; k++) SLIP_PUSH(p[k]);
    }
    const uint8_t *c = (const uint8_t *)&crc;
    for (size_t k = 0; k < sizeof(crc); k++) SLIP_PUSH(c[k]);

    buf[i++] = SLIP_END;

    // One write = one send() on TCP
    ssize_t sent = 0;
    while (sent < (ssize_t)i) {
        ssize_t n = write(t->fd, buf + sent, i - sent);
        if (n < 0) {
            // If interrupted by a signal or kernel buffer is full, back off and retry.
            if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK) {
                usleep(500); 
                continue;
            }
            // Hard error (e.g., PC disconnected, EPIPE)
            break; 
        }
...
void transport_send_text_resp(Transport *t, uint32_t seq, const char *text) {
    if (!text) text = "";
    transport_send(t, MSG_RESP, seq, 0, text, (uint32_t)strlen(text));
}
