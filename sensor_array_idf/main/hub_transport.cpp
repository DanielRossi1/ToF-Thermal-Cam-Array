...
Transport::Transport(uart_port_t uart) : uart_(uart) {}

void Transport::slipWriteByte(uint8_t b) {
  switch (b) {
    case kSlipEnd:
      uart_write_bytes(uart_, &kSlipEsc, 1);
      uart_write_bytes(uart_, &kSlipEscEnd, 1);
      break;
    case kSlipEsc:
      uart_write_bytes(uart_, &kSlipEsc, 1);
      uart_write_bytes(uart_, &kSlipEscEsc, 1);
      break;
    default:
      uart_write_bytes(uart_, &b, 1);
      break;
  }
}
...
bool Transport::send(MsgType type, uint32_t seq, uint64_t ts_us, const void* payload, uint32_t payload_len) {
  ...
  uart_write_bytes(uart_, &kSlipEnd, 1);
